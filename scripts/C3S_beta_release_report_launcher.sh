...
function bsub_jobs {
	bsub -J report_$sid_deck -oo $sid_deck_job_dir/$sid_deck"_report.o" -eo $sid_deck_job_dir/$sid_deck"_report.e" -q short-serial -W $job_hours:$job_minutes -M $memo -R "rusage[mem=$memo]" python $scripts_directory/report_tools/pdf_report.py $sid_deck $sid_deck_report_dir $first_year $last_year $sid_deck_report_dir/no_qc
}
...
logfile=$jobs_dir/${filebase%.*}_report_$(date +'%Y%m%d_%H%M').log
exec > $logfile 2>&1
