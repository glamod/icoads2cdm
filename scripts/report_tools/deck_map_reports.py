...
deck = '714'
file_path = '/Users/iregon/C3S/dessaps/CDSpy/test_data/ICOADS3_to_CDS'
y_ini = 1980
y_end = 1981
vars_in = ['sst','at']
...
descriptor_list = ['counts','ave','max','min']
...
def read_monthly_data():
    ...
    file_header = os.path.join(file_path,'-'.join(['header',str(yr),'{:02d}'.format(mo)]) + '.psv')
...
def map_on_subplot(subplot_ax):
    ...
    gl = subplot_ax.gridlines(crs=ccrs.PlateCarree(),color = 'k', linestyle = ':', linewidth = grid_width, alpha=0.5, draw_labels=True)
    ...
    cax = divider.new_horizontal(size = new_axis_w, pad = 0.1, axes_class=plt.Axes)
    f.add_axes(cax)
    if show_colorbar:
        ...
        cb_v = mpl.colorbar.ColorbarBase(cax, cmap=colormap,norm = norm, orientation='vertical')
        cb_v.set_label(colorbar_title, size = colorbar_title_size)
    ...
    for mo in range(1,13):
        ...
        for vari in vars_in:
            ...
            if 'global' in descriptors['counts'][vari].keys():
                ...
                descriptors['counts'][vari]['global'] = cvs.points(df_mo[['latitude','longitude',vari]], 'longitude', 'latitude', ds.count(vari))
                descriptors['max'][vari]['global'] = cvs.points(df_mo[['latitude','longitude',vari]], 'longitude', 'latitude', ds.max(vari))
                descriptors['min'][vari]['global'] = cvs.points(df_mo[['latitude','longitude',vari]], 'longitude', 'latitude', ds.min(vari))
...
for agg_period in ['global']: #descriptors['max'][vari].keys(): 
    ...
    for descriptor in descriptor_list:
        for vari in vars_in:
            ...
            colorbar_title = vars_units.get('counts') if descriptor == 'counts' else vars_units.get(vari)
            plt.title('-'.join([deck,vari]) + ": " + " ".join([agg_name,descriptor]) + '\n' + "-".join([str(y_ini),str(y_end)]) + ' composite')
            ...
            map_on_subplot(ax)
            plt.savefig('/Users/iregon/C3S/dessaps/CDSpy/L1a/maps/' + '-'.join([deck,vari,descriptor,agg_period,'qc']) + '.png',bbox_inches='tight')
            plt.close()
...
f, ax = plt.subplots(4, 2, subplot_kw=dict(projection=proj),figsize=(7,14),dpi = 180)  # With mosaic, to have a good resolution, have to increase dpi on savefig...and here state a "normal" one. If try to increase here, does strange, harmful things....
...
for agg_period in ['global']:  
    ...
    for descriptor in descriptor_list:
        ...
        for vari in vars_in:
            ...
            z = descriptors[descriptor][vari][agg_period].where(descriptors[descriptor][vari][agg_period]> 0).values
            if not qc_mode:
                show_colorbar = True if c == len(vars_in) - 1 else False
            map_on_subplot(ax[r,c])
            c += 1
        r += 1
    
    wspace = 0.25 if qc_mode else 0.05     
    ...
    f.savefig('/Users/iregon/C3S/dessaps/CDSpy/L1a/maps/' + '-'.join([deck,'mosaic',agg_period,'qc']) + '.png',bbox_inches='tight',dpi = 300)# 'tight' here, not in plt.tight_layout: here it realizes the new size because of add_axes, but not in plt.tight_layout....  
    plt.close()
#            
### Now map colorbars
##for vari in vars_in:
...
##    cb_v.ax.tick_params(labelsize = colorbar_label_size)
##    plt.savefig('/Users/iregon/C3S/dessaps/CDSpy/L1a/maps/' + '-'.join([str(deck),vari,'vertical_cb']) + '.png',bbox_inches='tight')
##    plt.close(i)
...
##    plt.savefig('/Users/iregon/C3S/dessaps/CDSpy/L1a/maps/' + '-'.join([str(deck),vari,'horizontal_cb']) + '.png',bbox_inches='tight')
##    plt.close(i)
##
##vari= 'counts'
