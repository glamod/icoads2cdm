...
try:
    ...
    out = sys.argv[5] # Specific to qc report
...
cvr_txt = []
cvr_txt.append('CDS BETA RELEASE')
cvr_txt.append('QC flagged POS, AT, SST, DPT and SLP')
...
cvr_properties = dict()
cvr_properties['size'] = [22,20,16,16,14,14]
cvr_properties['effects'] = ['B','B','','','I','']
cvr_properties['align'] = ['C','C','C','C','C','C']
...
for vari in vars_in:
    ...
    ts_path = os.path.join(inDir,"qc","_".join([sid_deck,vari,"lat_band_stats_ts.png"]))
    print(ts_path)
    if os.path.isfile(ts_path):
        ...
        pdf.ln()
    # 3. ADD MOSAIC
    map_path = os.path.join(inDir,"qc","_".join([sid_deck,vari,"mosaic_global_map.png"]))
...
pdf.output(os.path.join(out,sid_deck + '_report_qc.pdf'))
