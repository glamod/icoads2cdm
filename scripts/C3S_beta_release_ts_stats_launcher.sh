...
function bsub_jobs {
	bsub -J ts_stats_$sid_deck -oo $sid_deck_job_dir/$sid_deck"_ts_stats.o" -eo $sid_deck_job_dir/$sid_deck"_ts_stats.e" -q short-serial -W $job_hours:$job_minutes -M $memo -R "rusage[mem=$memo]" python $scripts_directory/report_tools/deck_ts_report_lat_bands.py $sid_deck $sid_deck_data_dir $first_year $last_year $sid_deck_report_dir $level
}
...
logfile=$jobs_dir/${filebase%.*}_ts_stats_$(date +'%Y%m%d_%H%M').log
exec > $logfile 2>&1
...
do
	...
	IFS=$OFS
	sid_deck=${process[0]}
	sid_deck_job_dir=$jobs_dir/$sid_deck/ts_stats
